...
public abstract class Card
{
    ...
    // --- 2. 생성자 (카드 처음 생성 시) ---
    /// 새 카드를 생성할 때 호출됩니다.
    public Card(object owner, int index, float cooldown, string cardID = null)
    {
        this.Owner = owner;
        ...
        // Auto-configure from Attribute
        var attr = this.GetType().GetCustomAttribute<CardConfigAttribute>();
        if (attr != null)
        {
            this.CardID = attr.Id; // Attribute takes precedence
            this.CardNameKey = $"card_{attr.Id}_name";
            this.CardSkillDescriptionKey = $"card_{attr.Id}_desc";
            this.FlavorTextKey = $"card_{attr.Id}_flavor";
        }
        else
        {
            if (string.IsNullOrEmpty(this.CardNameKey)) this.CardNameKey = "Default Card Name";
        }
